create.box.disease <- function(total.sub, gene, type, hist){
  
  # subset by gene and histology
  for.box <- total.sub[which(total.sub$gene == gene & total.sub$disease %in% hist),]
  ...
  for.box <- for.box %>% 
    ...
    group_by(study_id, disease) %>%
    mutate(median.fpkm = round(median(fpkm), digits = 2),
  ...
  # plot
  p <- ggplot(for.box, aes_string(x = 'label', y = val, alpha = 0.5)) + 
    stat_boxplot(geom ='errorbar', width = 0.2) +
    geom_boxplot(lwd = 0.5, fatten = 0.7, outlier.shape = NA, width = 0.5, aes(fill = study_id)) +
    theme_bw() + theme_Publication() + 
    xlab('') + ylab(type) +
    ggtitle("Expression Boxplot") + guides(alpha = FALSE)
  p <- plotly_build(p)
  
  # format data for display
  for.box <- for.box[,c('gene','sample_id','study_id','disease','disease_name','label','definition','fpkm','log2fpkm','z.score')]
  
  newList <- list(p, for.box)
  return(newList)
}
